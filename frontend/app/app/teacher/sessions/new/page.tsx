...
export default function NewSessionPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (
      data:
        | SessionCreate
        | { file: File; title: string; description?: string; classroom_id?: number; subject_id?: number; mapping_id?: number }
    ) => {
      if ('file' in data) {
    ...
    <RoleGuard allowedRoles={['teacher', 'admin', 'management']}>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Create a new recorded session</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Create Session</CardTitle>
            <CardDescription>Upload a recorded video and assign it to a class–subject–teacher mapping</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {user?.role === 'teacher' && mappingsForUser.length === 0 && (
                  <p className="text-sm text-muted-foreground">No classes assigned to you yet. Ask admin to add a mapping.</p>
                )}
              </div>
              <div className="space-y-2">
              ...
              {createMutation.isError && (
                <div className="rounded-md bg-destructive/10 text-destructive text-sm p-3">
                  {(() => {
                    ...
                    return msg || 'Upload failed. Check the Flask server console for the error.'
                  })()}
                </div>
              )}
